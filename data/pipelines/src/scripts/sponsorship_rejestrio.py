"""Resolve the sponsorship recipients to KRS numbers, and queue them for rejestr.io.

One command does the whole thing:

    uv run python src/scripts/sponsorship_rejestrio.py resolve

That searches the register for each NIP in `scrapers.krs.sponsorship_nips` and
...
from scrapers.krs import nip_lookup, nip_sources, search, sponsorship_nips
...
def resolve(args) -> None:
    ...
    if free:
        print(
            ...
            f"({sponsorship_nips.total_paid(free):,.0f} PLN)",
            file=sys.stderr,
        )
    ...
    print(
        ...
        f"{sponsorship_nips.total_paid(unresolved):,.2f} PLN"
    )
    ...
    print(
        ...
        f"{sponsorship_nips.cost_pln(len(mapping)):,.2f} PLN "
...
def already_bought(ctx: Context) -> set[tuple[str, str]]:
    ...
    for ref in ctx.io.list_files(CloudStorage(prefix=REJESTRIO_PREFIX)):
        ...
        which = tail.split("/")[0]
        if krs and which in sponsorship_nips.REJESTRIO_QUERIES:
...
def fetch(args) -> None:
    ...
    wanted: list[tuple[sponsorship_nips.Recipient, str, str]] = []
    skipped = 0
    for recipient, krs in pairs:
        for which in sponsorship_nips.REJESTRIO_QUERIES:
    ...
    print(
        ...
        f"{len(wanted) * sponsorship_nips.PLN_PER_CALL:,.2f} PLN"
    )
    if not wanted:
        return
    if not args.yes:
    ...
    print(
        ...
        f"{done * sponsorship_nips.PLN_PER_CALL:,.2f} PLN spent"
    )
...
def worklist(args) -> None:
    ...
    print(
        ...
        f"{sponsorship_nips.total_paid(missing):,.2f} PLN",
        file=sys.stderr,
    )
...
def urls(args) -> None:
    ...
    for recipient, krs in pairs:
        ...
        for url in sponsorship_nips.rejestrio_urls(
            krs, with_org_record=args.with_org_record
        ):
            print(url)
    per_company = len(sponsorship_nips.REJESTRIO_QUERIES) + (
        1 if args.with_org_record else 0
    )
    print(
        f"\n# {len(pairs)} companies x {per_company} calls = "
        f"{sponsorship_nips.cost_pln(len(pairs), args.with_org_record):,.2f} PLN",
        file=sys.stderr,
    )
