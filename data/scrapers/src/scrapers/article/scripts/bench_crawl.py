...
def _plot_per_minute(run_dir: Path, per_minute: list[dict[str, float | str]]) -> None:
    if not go or not pio or not per_minute:
        return
    timestamps = [
        datetime.fromisoformat(cast(str, entry["minute"]))
        for entry in per_minute
    ]
...
def _plot_sql_metrics(
    ...
    if sql_per_query:
        ...
        minute_timestamps = [
            datetime.fromisoformat(cast(str, minute))
...
def main() -> int:
    ...
    seed_count = run_meta.get("seed_count")
    if not isinstance(seed_count, int):
        raise ValueError("run metadata missing seed_count")
