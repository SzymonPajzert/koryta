...
def delete(verbose=False, confirm=False):
    def func(ctx: Context):
        ...
        if confirm and input("Are you sure? (y/n)") != "y":
            return
        ctx.batch.update(
            ctx.db.collection(collection).document(ctx.root_id),
            {path: firestore.DELETE_FIELD},
        )
...
def create_edge(ctx: Context, source_id, target_id, edge_type, **kwargs):
    ...
    ctx.batch.create(
        ctx.db.collection("edges").document(),
        document_data=dict(
            {
...
def create_and_link_blob(ctx: Context, root, value, config: BlobConfig):
    ...
    ctx.batch.create(
        blob_doc, document_data={"type": "blob", config.field_name: extracted_value}
    )
    ctx.batch.create(
        ctx.db.collection("edges").document(),
        document_data={
...
def create_link_or_blob(ctx: Context, root, value, config: EdgeConfig):
    if nested_access(config, value, "target") not in ["", None, {}]:
        ...
        create_and_link_blob(
            ctx, root, value, BlobConfig(config.edge_type, "text", "text")
        )
...
OPERATIONS = {
    ...
    "article.companies": for_each_value(
        create_link, EdgeConfig("mentions", field_mapping={"target": "id"})
    ),
    "article.people": for_each_value(
        create_link, EdgeConfig("mentions", field_mapping={"target": "id"})
    ),
    ...
    "place.owners": for_each_value(
        create_link, EdgeConfig("owns", revert=True, field_mapping={"target": "id"})
    ),
    "place.owner": for_value(
        create_link, EdgeConfig("owns", revert=True, field_mapping={"target": "id"})
    ),
    "place.manager": for_value(
        create_link, EdgeConfig("employed", revert=True, field_mapping={"target": "id"})
    ),
    "article.comments": for_each_value(
        create_and_link_blob, BlobConfig("comment", "text", "text")
    ),
    "person.comments": for_each_value(
        create_and_link_blob, BlobConfig("comment", "text", "text")
    ),
    "place.comments": for_each_value(
        create_and_link_blob, BlobConfig("comment", "text", "text")
    ),
    "person.sourceURL": for_value(
        create_and_link_blob, BlobConfig("source", "url", None)
    ),
}
        ...
        for doc in collection.stream():
            ...
            for key, operation in OPERATIONS.items():
                if key in nested_keys:
                    ...
                    try:
                        operation(
                            Context(key, value, firestore_db, bulk_writer, doc.id)
                        )
                    except KeyError:
...
def migrate_to_nodes(collections: list[str | tuple[str, str]]):
    ...
    for collection_name in collections:
        ...
        for doc in tqdm(
            docs, desc=f"Migrating {collection_name} to {collection_target}"
        ):
            data = doc.to_dict()
            ...
            new_node_ref = nodes_collection_ref.document(
                doc.id
            )  # Keep the same ID for now
