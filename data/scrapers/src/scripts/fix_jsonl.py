...
def convert_bucket_files_to_jsonl(source_bucket: str, destination_bucket: str):
    ...
    """
    print(f"--- Starting conversion for bucket: {source_bucket} to {destination_bucket}---")
    ...
    try:
        ...
        for source_blob in blobs:
            ...
            try:
                ...
                print(f"  [ERROR] Failed to parse JSON for file: {source_blob.name}. Skipping.")
                error_count += 1
            except Exception as e:
                print(f"  [ERROR] An unexpected error occurred with file {source_blob.name}: {e}. Skipping.")
                error_count += 1
