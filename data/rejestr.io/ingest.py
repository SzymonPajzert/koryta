...
def save_org_connections(krs: str, aktualnosc: Aktualnosc):
    ...
    for connection in connections:
        ...
        current_org.push(f"connections/{connection['id']}/relation", connection["krs_powiazania_kwerendowane"])
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            print(f"Will try to perform {len(orgs)} reads, which would cost {len(orgs) / 10} PLN [yN]")
            if input() != "y":
                break
            for krs in orgs:
