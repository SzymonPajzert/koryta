  ...
  <article class="k-card k-card--accent note-entry" data-testid="note-entry">
    ...
    <template v-if="!isEditing">
      <div v-if="hasHead" class="note-entry__head">
        ...
        <span v-else-if="source.url" class="note-entry__kind">
          {{ source.url }}
        </span>
    ...
    <template v-else>
      <v-chip-group v-model="kind" mandatory density="compact">
        <v-chip
      ...
      <v-expand-transition>
        <div v-if="editingUrl" class="d-flex mt-2">
          <v-text-field
            v-model="source.url"
            label="URL"
            density="compact"
