describe("New Node Navigation", () => {
  ...
  it("navigates to the new node edit page after creation", () => {
    ...
    cy.contains("label", "Treść (Markdown)").parent().find("textarea").type("This is a test person content.");
    ...
    cy.contains("label", "Nazwa").parent().find("input").should("have.value", newName);
    ...
    cy.contains("label", "Wyszukaj osobę").parent().find("input").click().type("Anna", { delay: 100 });
    cy.get(".v-list-item-title", { timeout: 15000 }).contains("Anna Nowak").should("be.visible").click();
  ...
  it("allows creating a node without content and verifies it is searchable", () => {
    ...
    cy.visit("/");
    cy.contains("label", "Szukaj osoby albo miejsca").parent().find("input").type(newName);
    // Wait for debounce and search results
    cy.contains(".v-list-item-title", newName, { timeout: 10000 }).should("be.visible");
