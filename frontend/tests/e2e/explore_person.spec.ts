...
test.describe("Explore Person", () => {
  test("displays region search queries without middle names", async ({ page }) => {
    ...
    const firstRowName = page.locator("tbody tr:first-child .text-primary.cursor-pointer").first();
    ...
    await expect(drawer.locator(".v-btn", { hasText: new RegExp(`^${nameText}$`) })).toBeVisible();
    ...
    if (count > 0) {
      ...
      await expect(drawer.locator(".v-btn", { hasText: new RegExp(`^${expectedButtonText}$`) })).toBeVisible();
    }
  });
});
