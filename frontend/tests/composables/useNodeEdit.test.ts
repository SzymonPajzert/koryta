...
vi.mock("firebase/firestore", () => ({
    getFirestore: vi.fn(),
    doc: vi.fn(),
    getDoc: vi.fn(() => Promise.resolve({ exists: () => true, data: () => ({ type: "person" }) })),
}));
...
describe("useNodeEdit", () => {
    ...
    it("initializes for new node", async () => {
        ...
        const { isNew, current } = await useNodeEdit({ route: mockRoute, idToken: mockIdToken });
    ...
    it("fetches data for existing node", async () => {
        ...
        mockedFetch.mockResolvedValueOnce({ node: { name: "Test Person", type: "person" } });
        ...
        expect(mockedFetch).toHaveBeenNthCalledWith(1, "/api/nodes/entry/123", expect.anything());
    ...
    it("saves new node", async () => {
        ...
        expect(mockedFetch).toHaveBeenCalledWith("/api/nodes/create", expect.objectContaining({
            method: "POST",
    ...
    it("saves existing node as revision", async () => {
        ...
        const { saveNode, current } = await useNodeEdit({ route: mockRoute, idToken: mockIdToken });
        await new Promise(resolve => setTimeout(resolve, 50));
        ...
        expect(mockedFetch).toHaveBeenCalledWith("/api/revisions/create", expect.objectContaining({
            method: "POST",
    ...
    it("adds edge", async () => {
        ...
        const { addEdge, newEdge, pickerTarget } = await useNodeEdit({ route: mockRoute, idToken: mockIdToken });
        await new Promise(resolve => setTimeout(resolve, 50));
        mockedFetch.mockClear();
        ...
        expect(mockedFetch).toHaveBeenCalledWith("/api/edges/create", expect.objectContaining({
