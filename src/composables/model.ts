...
export function fillBlanks<D extends Destination>(value: Partial<DestinationTypeMap[D]>, d: D): Required<DestinationTypeMap[D]>
export function fillBlanks<D extends Destination>(valueUntyped: Partial<DestinationTypeMap[D]>, d: D) {
  if (d == "company") {
  ...
  if (d == "employed") {
    ...
    const result: Required<DestinationTypeMap["employed"]> = {
        ...
        employments: value.employments ?? recordOf({ text: "", relation: "", connection: new Link("company", "", "") }),
        connections: value.connections ?? recordOf({ text: "", relation: "", connection: new Link("employed", "", "") }),
        todos: value.todos ?? recordOf(new Link("todo", "", "")),
        comments: value.comments ?? recordOf({ text: "" }),
    };
    return result
  ...
  if (d == "data") {
    ...
    const result: Required<DestinationTypeMap["data"]> = {
        ...
        status: value.status ?? { tags: [], signedUp: {}, markedDone: {}, confirmedDone: false },
        todos: value.todos ?? recordOf(new Link("todo", "", "")),
        comments: value.comments ?? recordOf({ text: "" }),
    }
    return result
  ...
  if (d == "external/rejestr-io/krs") {
    ...
    const result: Required<DestinationTypeMap["external/rejestr-io/krs"]> = {
        name: value.name ?? "",
        connections: value.connections ?? recordOf({ state: "aktualne", type: "person" }),
        external_basic: value.external_basic ?? { id: "", nazwy: { skrocona: "" } },
  ...
  if (d == "external/rejestr-io/person") {
    ...
    const result: Required<DestinationTypeMap["external/rejestr-io/person"]> = {
        name: value.name ?? "",
        external_basic: value.external_basic ?? { id: "", state: "aktualne", tozsamosc: { data_urodzenia: "" } },
